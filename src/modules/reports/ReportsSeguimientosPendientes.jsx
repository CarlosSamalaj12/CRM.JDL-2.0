      ...
      <div className="reports-page-body">
        ...
        {filteredUserData.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            {filteredUserData.map((user, uIdx) => (
              <section key={user.userId} className="reports-hero-panel" style={{ gap: '10px' }}>
                <div className="reports-section-intro">
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', width: '100%' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <span className="reports-eyebrow" style={{ fontSize: '10px' }}>Vendedor</span>
                      <h3 className="reports-section-title" style={{ fontSize: '18px', margin: 0 }}>{user.name}</h3>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '22px', fontWeight: 900, color: '#0f172a', lineHeight: 1 }}>{user.total}</div>
                      <div style={{ fontSize: '10px', color: '#64748b', fontWeight: 600 }}>{user.total === 1 ? 'pendiente' : 'pendientes'}</div>
                    </div>
                  </div>
                </div>

                {/* Status distribution bar for this user */}
                <div style={{ height: '8px', borderRadius: '999px', background: '#f1f5f9', display: 'flex', gap: '3px', overflow: 'hidden', boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.04)' }}>
                  {PENDING_STATUSES.map(s => {
